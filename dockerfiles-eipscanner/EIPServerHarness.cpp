...
void signal_handler(int signum) {
    std::cerr << "Received signal " << signum << ", shutting down gracefully..." << std::endl;
    g_running = false;
...
int main(int argc, char* argv[]) {
    ...
    std::cerr << "EIPScanner server started on port " << port << std::endl;
    
    // Main server loop - continuously accept and handle connections
    while (g_running) {
        // Set accept timeout to allow checking g_running flag periodically
        ...
        if (client_sock < 0) {
            // Check if timeout or real error
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                // Timeout - check if we should continue running
                continue;
            }
            // Real error or shutdown signal
            if (g_running) {
        ...
        if (bytes_received <= 0) {
            close(client_sock);
            continue;  // Continue to next connection instead of exiting
        }
        ...
        // Flush coverage data after each connection to avoid data loss on crash
#ifdef __GNUC__
        __gcov_flush();
#endif
    ...
    // Flush coverage data one more time
#ifdef __GNUC__
    __gcov_flush();
#endif
