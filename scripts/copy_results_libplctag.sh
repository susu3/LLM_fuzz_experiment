...
# 从容器中拷贝 libplctag 模糊测试结果文件

RUN_NUMBER="${1:-1}"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTPUT_DIR="./copied_results_libplctag_run${RUN_NUMBER}_${TIMESTAMP}"

echo "拷贝 libplctag 第 $RUN_NUMBER 次实验的结果..."
echo "创建输出目录: $OUTPUT_DIR"
mkdir -p "$OUTPUT_DIR"

CONTAINERS=("afl-ics-libplctag" "aflnet-libplctag" "chatafl-libplctag" "a2-libplctag" "a3-libplctag")
TOOLS=("afl-ics" "aflnet" "chatafl" "a2" "a3")

for i in "${!CONTAINERS[@]}"; do
    container="${CONTAINERS[$i]}"
    tool="${TOOLS[$i]}"
    result_dir="libplctag-${tool}-${RUN_NUMBER}"
